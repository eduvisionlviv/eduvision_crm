...
def _find_accounts_by_email(email: str) -> List[dict]:
    """Пошук акаунтів за email у всіх таблицях.

    Supabase `eq` — case sensitive, тому робимо подвійний запит: спочатку
    прямий збіг, потім `ilike`, щоб зловити записи з різним кейсом або з
    випадковими пробілами в БД.
    """
    ...
    for table in LOGIN_TABLES:
        ...
        try:
            ...
            log.info("lookup failed for %s in %s (eq): %s", _mask_email(email), table, exc)

        # 2) fallback: case-insensitive, якщо нічого не знайшли
        if not rows:
            try:
                res = client.table(table).select("*").ilike("user_email", email).execute()
                rows = res.data or []
            except Exception as exc:
                log.info("lookup failed for %s in %s (ilike): %s", _mask_email(email), table, exc)

        for row in rows:
            norm_row = _normalize_account(table, row)
            stored_email = (norm_row.get("user_email") or "").strip().lower()
            if stored_email != email:
                norm_row["user_email"] = stored_email
