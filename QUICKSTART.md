...
- **Telegram-бот не працює?**
  ...
  2. Додайте `TELEGRAM_BOT_TOKEN=<токен від BotFather>` (можна також `BOT_TOKEN` або `TELEGRAM_TOKEN`) і, за потреби, `TELEGRAM_PROXY=<http(s)://user:pass@host:port>` у змінні середовища; перезапустіть бекенд. Без токена бот не стартує.
  3. Якщо `/api/tg/status` показує `status: error`, токен може бути невірним або мережа блокує доступ до api.telegram.org. Бот сам перепідключається з паузою 10 секунд і чистить «завислі» апдейти.
