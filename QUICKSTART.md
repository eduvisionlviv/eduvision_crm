...
- **Telegram-бот не працює?**
  ...
  2. Додайте `TELEGRAM_BOT_TOKEN=<токен від BotFather>` і, за потреби, `TELEGRAM_PROXY=<http(s)://user:pass@host:port>` у змінні середовища; перезапустіть бекенд. Без токена бот не стартує.
